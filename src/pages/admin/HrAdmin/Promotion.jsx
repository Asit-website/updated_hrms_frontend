...
const Promotion = () => {
  ...
  const { getPromotion, promotion, createPromotion, deletePromotion, updatePromotion, allEmp, allDesignation } = useMain();
          ...
          buttons: [
            {
              ...
              onClick: async () => {
                let res = await deletePromotion(item?._id);
                if (res.success) {
                  await getPromotion();
  ...
  const fields = [
    ...
    },
    {
      ...
      label: "Designation",
      type: "select",
      options: allDesignation?.map((emp) => ({
        value: emp?.name,
        label: emp?.name,
      })),
